...
export default function PersistentDeploymentProgress({
  installationId,
  deploymentStatus,
}) {
  ...
  const shouldShow =
    deploymentStatus &&
    ["starting", "in_progress", "completed", "failed"].includes(
      deploymentStatus
    );
  ...
  useEffect(() => {
    if (shouldShow && installationId) {
      ...
      console.log(
        "Connecting to WebSocket for deployment tracking:",
        socketUrl
      );
      ...
      newSocket.on("connect_error", (error) => {
        console.error(
          "WebSocket connection error for deployment tracking:",
          error
        );
      });
        ...
        <div className="px-4 sm:px-6 py-4">
          ...
          {showLogs && (
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              ...
              <div className="bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg p-4 max-h-96 overflow-y-auto">
              ...
              <div className="flex justify-between items-center">
                {hasFailed && !isComplete && (
                  <p className="text-sm text-orange-600 dark:text-orange-400">
                    Deployment encountered issues. Check the logs above for
                    details.
                  </p>
                )}
                {isComplete && (
                  <p className="text-sm text-green-600 dark:text-green-400">
                    Deployment completed successfully! Your application should
                    now be available.
                  </p>
                )}
              </div>
