...
export async function removeNginxConfig(subdomain) {
  try {
    ...
    // Remove ALL server blocks for this domain using a Python script for better handling
...
with open('/etc/nginx/sites-available/sciphr', 'r') as f:
    content = f.read()

# Pattern to match any server block that contains our domain
domain = '${domain}'
subdomain = '${subdomain}'
...
while i < len(lines):
    ...
    if f'# {subdomain} - Auto-generated by SaaS deployment' in lines[i]:
        skip_block = True
        i += 1
        continue
    
    # Check for server block start
    if 'server {' in lines[i] and not skip_block:
        # Look ahead to see if this server block contains our domain
        temp_i = i
        temp_brace_count = 1
        contains_domain = False
        
        while temp_i + 1 < len(lines) and temp_brace_count > 0:
            temp_i += 1
            temp_line = lines[temp_i]
            if '{' in temp_line:
                temp_brace_count += temp_line.count('{')
            if '}' in temp_line:
                temp_brace_count -= temp_line.count('}')
            if domain in temp_line:
    ...
    # Handle skipping logic for blocks we want to remove
    if skip_block:
        if '{' in lines[i]:
...
with open('/tmp/sciphr.tmp', 'w') as f:
    f.write('\\n'.join(output_lines))
"
    sudo mv /tmp/sciphr.tmp /etc/nginx/sites-available/sciphr
    `;
